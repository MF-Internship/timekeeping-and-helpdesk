...
async function checksum(file: File) {
  ...
  return [...new Uint8Array(bytes)]
    .map((value) => value.toString(HEX_RADIX).padStart(HEX_WIDTH, "0"))
    .join("");
}
...
export function FieldEvidenceForm(props: {
  ...
  return (
    <form
      ...
      onSubmit={submit}
      aria-label={`Nộp minh chứng ${props.taskTitle}`}
    >
      <div className={styles.formHeading}>
        <Camera size={20} aria-hidden="true" />
        <div>
          <strong>Minh chứng hoàn thành</strong>
      ...
      <ActionGroup>
        <Button type="submit" variant="primary" loading={props.busy || Boolean(stage)}>
          Nộp minh chứng & hoàn thành
        </Button>
        <Button type="button" variant="quiet" onClick={draft.discard}>
          Bỏ bản nháp
        </Button>
      </ActionGroup>
    </form>
...
function DraftNotice({
  ...
  if (persistence.kind === "ready")
    return (
      <p role="status">Đã khôi phục {files.length} ảnh và ghi chú từ bản nháp của task này.</p>
    );
  if (persistence.kind === "quota")
    return <p role="alert">Bộ nhớ trình duyệt đã đầy; bản nháp chưa được lưu.</p>;
  if (persistence.kind === "unavailable")
    return <p role="alert">Không thể dùng bộ nhớ trình duyệt; bản nháp chưa được lưu.</p>;
  if (persistence.kind === "evicted")
    return <p role="alert">Bản nháp cục bộ đã bị trình duyệt xóa.</p>;
  return null;
}
...
function SubmissionFeedback({ stage, error }: { stage?: string; error?: string }) {
  return (
    <>
      {stage && (
        <p role="status">
          <LocateFixed size={16} aria-hidden="true" /> {stage}
        </p>
      )}
  ...
  setError(
    gpsFailure
      ...
      : reason instanceof Error
        ? reason.message
        : "Không thể nộp minh chứng.",
  );
}

function validateFiles(files: readonly File[], uploadIds: readonly string[]) {
  if ((!files.length && !uploadIds.length) || files.length > MAX_FILES)
    return "Chọn từ 1 đến 5 ảnh minh chứng.";
...
function completionBody(
  ...
  note: string,
  selected?: number,
): taskApi.TaskFieldCompletionInput {
  return {
...
function locationCandidates(reason: unknown): Candidate[] {
  ...
  return values.filter(
    (item): item is Candidate =>
      typeof item === "object" &&
      item !== null &&
      typeof (item as Candidate).id === "number" &&
      typeof (item as Candidate).code === "string" &&
...
async function uploadFiles(
  ...
  for (const file of files.slice(completed.length)) {
    const intent = await taskApi.createEvidenceUpload(taskId, {
      ...
      mime: file.type as taskApi.EvidenceUploadInput["mime"],
