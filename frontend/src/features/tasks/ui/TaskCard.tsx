...
function SelectedEditor(
  props: Parameters<typeof TaskCard>[0] & {
    editor?: "content" | "status" | "override" | "evidence" | "delete";
    busy: boolean;
  },
) {
  if (props.editor === "content") return <TaskEditor {...props} />;
  if (props.editor === "status")
    return (
      <TaskStatusForm
        task={props.task}
        busy={props.busy}
      ...
      <dl className={styles.metadata}>
        ...
        {task.overdue_days ? (
          <div>
            <dt>Tiến độ</dt>
            <dd>
              <StatusBadge tone="critical">Quá hạn {task.overdue_days} ngày</StatusBadge>
            </dd>
          </div>
        ) : null}
        <div className={styles.metadataItem}>
...
function EvidenceAction(props: Parameters<typeof TaskActions>[0]) {
  return props.capabilities.canCompleteField ? (
    <Button variant="primary" onClick={() => props.onSelect("evidence")}>
      Nộp minh chứng
    </Button>
  ) : null;
}
...
function DeleteAction(props: Parameters<typeof TaskActions>[0]) {
  return canDeleteTask(props) ? (
    <Button variant="destructive" onClick={() => props.onSelect("delete")}>
      Xóa task tự tạo
    </Button>
  ) : null;
}
...
function OverrideAction(props: Parameters<typeof TaskActions>[0]) {
  return props.capabilities.canOverride ? (
    <Button variant="destructive" onClick={() => props.onSelect("override")}>
      Hoàn thành
    </Button>
  ) : null;
}
...
function canDeleteTask(
  props: Pick<Parameters<typeof TaskActions>[0], "capabilities" | "task" | "accountId">,
) {
  return (
    props.capabilities.canDeleteSelf &&
...
function UpdateActions({ onSelect }: Pick<Parameters<typeof TaskActions>[0], "onSelect">) {
  return (
    <>
      <Button onClick={() => onSelect("content")}>Sửa nội dung</Button>
      <Button onClick={() => onSelect("status")}>Đổi trạng thái</Button>
    </>
  );
}
