...
**Purpose**: Close the remaining gaps introduced by the synchronized R-110–R-112 logout, no-op/revocation-evidence, throttle, concurrency, and verification contracts.

- [X] T137 CRITICAL correct logout in `backend/identity/adapters/api/permissions.py`, `backend/identity/adapters/api/views.py`, `backend/identity/adapters/api/urls.py`, `backend/config/composition.py`, and `backend/identity/application/authentication.py` so valid bearer access is the sole actor authority; remove refresh-cookie target authorization, always call actor-wide revocation, clear the canonical cookie, and return `204` for missing, malformed, expired, revoked, mismatched-owner, or active cookies without issuing credentials; replace the contradictory logout API/unit/contract expectations with authorization-order, body-validation, all-cookie-state, multi-device, and no-forbidden-side-effect coverage per FR-008 and US1/AC7 (contradicts)
- [X] T138 implement R-111 no-op and conditional evidence semantics across `backend/identity/application/authentication.py`, `backend/identity/application/self_service.py`, and `backend/identity/application/user_admin.py`: return the current representation without User writes, revocation, evidence, or aggregate-version advance for same-state status; append session-revocation AuditLog/OutboxEvent only when `revoked_count > 0` for logout, reset, self password change, and deactivation while preserving a repeated reset as a new password mutation; prove exact state, call, evidence, rollback, and version behavior in unit, API, and PostgreSQL tests per FR-044 and FR-045 (contradicts)
- [X] T139 add fail-closed DRF authentication throttles using `core.cache.THROTTLE_CACHE_ALIAS` for login at 10 requests per 60 seconds per canonical client IP, refresh at 120 per 60 seconds per canonical client IP, and password change at 5 per 60 seconds per authenticated User after its authorization/account gates; add canonical `THROTTLED` and `SERVICE_UNAVAILABLE` envelopes, `Retry-After`, exact controlled-clock boundary/key-isolation/shared-worker/cache-failure tests, and assertions that over-limit or store-failure paths never invoke business services or append evidence per FR-046 and SC-015 (missing)
- [X] T140 synchronize the authentication-throttle wire and frontend contracts by annotating `429`/`503` responses, deterministically regenerating `contracts/openapi.yaml` and `frontend/src/shared/api/schema.ts`, extending backend contract/safety tests, and teaching `frontend/src/shared/errors/api-error.ts`, `frontend/src/shared/messages.ts`, and identity UI flows to recognize `THROTTLED`/`SERVICE_UNAVAILABLE`, honor and present `Retry-After`, avoid premature automatic retry, and retain request IDs per plan: API/frontend throttle contract (missing)
- [X] T141 expand the real PostgreSQL serialization proof so login and refresh each race with logout, Manager reset, self password change, and deactivation in both lock orders with production services, deterministic lock-boundary synchronization, separate connections, and complete User/token/evidence/final-access assertions; replace the adapter-level/manual-evidence concurrent global-revocation test with the shared production revocation path and prove one positive-count evidence pair followed by a zero-count no-op, while preserving valid genuinely-later issuance semantics per FR-043 and SC-013 (partial)
- [X] T142 add controlled-time access-expiry coverage parameterized across logout, Manager reset, self password change, and deactivation, proving cryptographic acceptance immediately before 15-minute expiry and `INVALID_TOKEN` at or after expiry while also proving the operation-specific pre-expiry outcomes (`200`, `PASSWORD_CHANGE_REQUIRED`, or `ACCOUNT_INACTIVE`) without sleeps per plan: controlled-time access-expiry coverage (partial)
- [X] T143 wire the new logout/no-op/throttle/dual-order-race/access-expiry suites into the existing `.github/workflows/quality.yml`, `.pre-commit-config.yaml`, and `scripts/check_all.sh` gates with removal-detection contract tests, then run the focused backend/API/PostgreSQL/frontend/generated-contract checks and the complete Feature 002 verification gate without weakening assertions per plan: CI / Verification (missing)

## Phase 12: Convergence

**Purpose**: Close the remaining shared-worker authentication-throttle correctness and verification gap.

- [X] T144 replace the non-atomic `SimpleRateThrottle` cache-history read/write path with a shared-worker-safe counter/window implementation using only `core.cache.THROTTLE_CACHE_ALIAS` and the approved cache infrastructure, while preserving the exact login 10/60s per canonical client IP, refresh 120/60s per canonical client IP, and password-change 5/60s per authenticated User semantics, key isolation, canonical `429 THROTTLED` plus `Retry-After`, and fail-closed `503 SERVICE_UNAVAILABLE`; add real competing-worker integration coverage against the shared cache proving one common quota without lost counts or per-worker budgets and retaining zero business/audit/outbox side effects per FR-046 and SC-015 (partial)
