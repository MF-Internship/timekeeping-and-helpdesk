...
| Quality/Contract action warnings | Checkout, Node setup, and uv setup action majors still targeted GitHub's deprecated Node 20 action runtime | Pin current supported releases (`checkout@v7.0.1`, `setup-node@v7.0.0`, `setup-uv@v10.0.1`) without changing project Node/Python versions | Remote rerun pending |
| Contract #59 action resolution | Unlike the GitHub-owned actions, setup-uv does not publish a moving `v10` alias | Pin its published exact `v10.0.1` tag and use exact releases consistently for all actions | Corrected workflow pending remote run |
